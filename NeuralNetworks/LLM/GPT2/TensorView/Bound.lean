...
-/
theorem computeHelper_bounds_FINAL
    ...
    if h_rank_zero : rank = 0 then
      ...
      if h_rank_one : rank = 1 then
        ...
        · rw [if_neg h_idx_ge_dim] at h_helper_main_zero_i_rev
          ...
          apply computeHelper_bounds_FINAL shape rank h_rank_eq_size h_dims_pos_main indices h_indices_size 1 new_flatIndex_rec new_stride_rec
  ...
  | succ i_prev =>
    ...
    ·
      ...
      · rw [if_neg h_cond_idx_ge_dim_manual] at h_helper_main
        ...
        apply computeHelper_bounds_FINAL shape rank h_rank_eq_size h_dims_pos_main indices h_indices_size (i_rev + 1) new_flatIndex_rec new_stride_rec
...
This theorem establishes an upper bound for the `flatIndexVal`, guaranteeing it falls
...
- `h_rank_eq_size`: A proof that `rank` is equal to `shape.size`.
- `h_dims_pos`: A proof that all dimension sizes in `shape` are strictly positive.
  This ensures the total number of elements is positive.
- `indices`: An `Array Nat` containing the multi-dimensional coordinates for which
  the flat index is computed.
- `flatIndexVal`: The `Nat` value representing the computed flat index.
- `h_compute_main`: A proof that `computeFlatIndex` (with the given `shape`, `rank`,
  `h_rank_eq_size`, `h_dims_pos`, and `indices`) successfully returns `Except.ok flatIndexVal`.
-/
theorem computeFlatIndex_bounds
  ...
  apply computeHelper_bounds_FINAL shape rank h_rank_eq_size h_dims_pos indices h_indices_size 0 0 1
  · intro h_zero_eq_zero; rfl
  · exact Nat.zero_le rank
  · rw [Nat.sub_zero]; apply Eq.symm; exact extract_empty_product shape rank
  · simp; -- Handles `if i_rev > 0 then ... else True`. Since i_rev=0, it's True.
