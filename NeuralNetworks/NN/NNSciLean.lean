...
class Diffeology (X : Type u) where
  ...
  plotComp_eval {n m} (p : Plot n) (f : Rn m → Rn n) (hf : ContDiff ℝ ⊤ f) (u : Rn m) :
    plotEval (plotComp p f hf) u = plotEval p (f u)
...
class TangentSpace (X : Type u) [Diffeology X] (TX : outParam (X → Type v))
  ...
  tangentMap_comp {n m} (p : Diffeology.Plot X n) (f : Rn m → Rn n) (hf : ContDiff ℝ ⊤ f) (u : Rn m) :
    let df_u : Rn m →ₗ[ℝ] Rn n := (fderiv ℝ f u)
  ...
  tangentMap_exp_at_zero (v : Σ x, TX x) :
    (tangentMap (exp v) 0).2 (fun _ => 1) =
    Eq.mpr (congrArg TX (tangentMap_fst (exp v) 0 ▸ exp_at_zero v)) v.2
...
noncomputable def differential {X Y : Type*} [Diffeology X] [Diffeology Y]
  ...
  (f : X → Y) (hf : Diffeology.IsSmooth f) (x : X) (dx : TX x) : TY (f x) :=
  ...
  let p_Y := Diffeology.liftPlot f hf p_X
  -- 3. The differential is the velocity of the lifted plot at t=0.
  (TangentSpace.tangentMap p_Y 0).2 (fun _ => 1)
    |> Eq.mp ( by
          have h1 := TangentSpace.tangentMap_fst p_Y 0
          have h2 := Diffeology.liftPlot_spec f hf p_X
          have h3 := TangentSpace.exp_at_zero ⟨x, dx⟩
          simp [h1]
          simp_all only [Function.comp_apply, p_Y, p_X])
...
class AdjointSpace (K : Type*) (E : Type*) [RCLike K] [NormedAddCommGroup E] [NormedSpace K E] [CompleteSpace E] where
  ...
  smul_left : ∀ x y (r : K), toInner.inner (r • x) y = star r * toInner.inner x y
  conj_symm : ∀ x y, star (toInner.inner y x) = toInner.inner x y
...
def adjoint {K : Type*} [RCLike K] {E F : Type*}
    ...
    (f : E →L[K] F) : F →L[K] E :=
  -- Use Riesz representation: for each y ∈ F, find unique x ∈ E such that
  -- ⟪x, z⟫_E = ⟪y, f z⟫_F for all z ∈ E
  sorry -- This requires implementing the Riesz map for AdjointSpace
...
-- Make sure Function lemmas are available for simp
open Function
instance instStructTypePi (I : Type v) (E : I → Type w) [DecidableEq I] :
  ...
  structProj_structModify_ne := by
    intro i j h g f;
    intro x
    simp_all only [ne_eq, id_eq, not_false_eq_true, update_of_ne]
...
instance instStructTypeProd {X Y I_X I_Y} {XI : I_X → Type w} {YI : I_Y → Type w}
    [DecidableEq I_X] [DecidableEq I_Y]
    [StructType X I_X XI] [StructType Y I_Y YI] :
    StructType (X × Y) (I_X ⊕ I_Y) (fun i => match i with | Sum.inl ix => XI ix | Sum.inr iy => YI iy) where

  structProj := fun (x, y) i => match i with
    | Sum.inl ix => structProj x ix
    | Sum.inr iy => structProj y iy

  structMake := fun f =>
    (structMake (fun ix => f (Sum.inl ix)), structMake (fun iy => f (Sum.inr iy)))
  ...
  -- structMake ∘ structProj = id on component functions
  left_inv := by
    intro f
    funext i
    cases i with
  ...
  -- structProj ∘ structMake = id on pairs
  right_inv := by
    intro xy
    cases xy with
    | mk x y =>
      simp only
      rw [right_inv, right_inv]

  structProj_structModify_eq := by
    intro _ i f xy
    cases i with
    | inl ix =>
      simp [StructType.structProj_structModify_eq]
    | inr iy =>
      simp [StructType.structProj_structModify_eq]
  ...
  structProj_structModify_ne := by
    ...
    | inl ix =>
      cases j with
      | inl jx =>
        have h' : jx ≠ ix := fun h_eq => h (by cases h_eq; rfl)
        simp [StructType.structProj_structModify_ne h']
      | inr jy =>
        simp
    | inr iy =>
      cases j with
      | inl jx =>
        simp
      | inr jy =>
        have h' : jy ≠ iy := fun h_eq => h (by cases h_eq; rfl)
        simp [StructType.structProj_structModify_ne h']
